...
|---|---:|---|
| **Installs of latest release** (`v0.1.3`) | **7** | Bottles pulled for the newest tag. The most honest install number we have. |
| All-time install events | 34 | Sum of every bottle download across every release. **Inflated by `brew upgrade` churn — not a user count.** |
...
Each daily snapshot is one line in [`install-stats.ndjson`](install-stats.ndjson). The file is append-only (deduped by date so re-running the script on the same day overwrites that day's row). Total snapshots so far: **1**.
