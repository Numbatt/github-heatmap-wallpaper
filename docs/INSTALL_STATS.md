...
> *Auto-generated from `docs/install-stats.ndjson`. Last snapshot: **2026-07-09T06:55:49Z**. Do not hand-edit — re-run `script/install-stats.sh` to regenerate.*
...
| All-time install events | 89 | Sum of every bottle download across every release. **Inflated by `brew upgrade` churn — not a user count.** |
...
Each daily snapshot is one line in [`install-stats.ndjson`](install-stats.ndjson). The file is append-only (deduped by date so re-running the script on the same day overwrites that day's row). Total snapshots so far: **66**.
