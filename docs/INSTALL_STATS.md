...
> *Auto-generated from `docs/install-stats.ndjson`. Last snapshot: **2026-09-02T07:57:53Z**. Do not hand-edit — re-run `script/install-stats.sh` to regenerate.*
...
Each daily snapshot is one line in [`install-stats.ndjson`](install-stats.ndjson). The file is append-only (deduped by date so re-running the script on the same day overwrites that day's row). Total snapshots so far: **121**.
